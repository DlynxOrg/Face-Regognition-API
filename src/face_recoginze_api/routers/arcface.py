...
from face_recoginze_api.services.arcface_recognize_service import ArcFaceRecognizeService, ErrorType
database = Database()
...
async def lifespan(app: FastAPI):
    ...
    async with database.get_session() as db_session:  # ✅ Dùng async with để lấy AsyncSession
        faceRecognizeService = ArcFaceRecognizeService()
...
async def recognize_face(image_id: int, session: AsyncSession = Depends(database.get_session)):
    async with session as db_session:
        error, result = await faceRecognizeService.recognize_face_faiss_arcface(image_id=image_id, db=db_session)
